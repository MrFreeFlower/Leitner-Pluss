...
export const RenameDeckModal: React.FC<RenameDeckModalProps> = ({ isOpen, onClose, onRename, currentName, theme }) => {
  const [name, setName] = currentName ? useState(currentName) : useState('');
    ...
    <div className="fixed inset-0 z-[60] bg-[#F3F5F9] flex flex-col animate-in slide-in-from-bottom duration-300">
      <div className="flex flex-col h-full p-6">
        ...
        <div className="relative flex items-center justify-center mb-8 pt-2">
            ...
            <h2 className="text-2xl font-normal text-slate-900">Rename Box</h2>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1">
            ...
            <div className="relative mb-1">
                <input
                    ...
                    className={`w-full bg-white px-5 py-4 rounded-2xl text-slate-900 placeholder:text-slate-400 text-lg focus:outline-none shadow-sm transition-colors border-2 ${name.trim() ? classes.border : 'border-transparent'} ${classes.border.replace('border-', 'focus:border-')}`}
                />
            </div>
